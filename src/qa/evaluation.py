...
def calculate_matches(data: List, workers_num: int, match_type: str='string'):
    """
    ...
    get_score_partial = partial(check_answer, tokenizer=tokenizer, match_type=match_type)
...
def check_answer(example, tokenizer, match_type) -> List[bool]:
    ...
    for i, doc in enumerate(ctxs):
        ...
        if text is None:  # cannot find the document for some reason
            ...
            hits.append(False)
            continue
        hits.append(has_answer_with_type(answers, text, tokenizer, match_type))
...
def has_answer_with_type(answers, text, tokenizer, match_type) -> bool:
    """Check if a document contains an answer string.
