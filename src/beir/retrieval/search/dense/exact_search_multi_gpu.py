...
if importlib.util.find_spec("evaluate") is not None:
    ...
    class DummyMetric(EvaluationModule):
            ...
            return EvaluationModuleInfo(
                ...
                features=Features(
                    {"cos_scores_top_k_values": Array2D((None, self.len_queries), "float32"),
                     "cos_scores_top_k_idx": Array2D((None, self.len_queries), "int32"), "batch_index": Value("int32")},
                ),
            )
        ...
        def warmup(self):
            ...
            """
            self.add_batch(cos_scores_top_k_values=torch.ones((1, 1, self.len_queries), dtype=torch.float32),
...
class DenseRetrievalParallelExactSearch:
    ...
    def __init__(self, model, batch_size: int = 128, corpus_chunk_size: int = None, target_devices: List[str] = None,
                 **kwargs):
    ...
    def search(self,
        ...
        self.corpus_chunk_size = min(math.ceil(len(corpus) / len(self.target_devices) / 10),
                                     5000) if self.corpus_chunk_size is None else self.corpus_chunk_size
        self.corpus_chunk_size = min(self.corpus_chunk_size,
                                     len(corpus) - 1)  # to avoid getting error in metric.compute()

        # @memray, in case that input is a dict, not a HF dataset
        if isinstance(corpus, dict):
            corpus = [{'id':k, 'title': v['title'] if 'title' in v else '', 'text': v['text']} for k,v in corpus.items()]
            queries = [{'id':k, 'text': v} for k,v in queries.items()]
        if self.sort_corpus:
            logger.info("Sorting Corpus by document length (Longest first)...")
            corpus = sorted(corpus, key=lambda x:len(x.get("title", "") + x.get("text", "")), reverse=True)
        ...
        for step, queries_batch in enumerate(queries_dl):
            with torch.no_grad():
                q_embeds = self.model.encode_queries(
                    queries_batch['text'], batch_size=self.batch_size, show_progress_bar=self.show_progress_bar,
        ...
        metric.filelock = FileLock(os.path.join(metric.data_dir,
                                                f"{metric.experiment_id}-{metric.num_process}-{metric.process_id}.arrow.lock"))
        metric.cache_file_name = os.path.join(metric.data_dir,
    ...
    def _encode_multi_process_worker(self, process_id, device, model, input_queue, results_queue):
        ...
        metric = DummyMetric(experiment_id=self.experiment_id, num_process=len(self.target_devices),
            ...
            while True:
                try:
                    ...
                    cos_scores = self.score_functions[self.score_function](
                        self.query_embeddings.to(corpus_embeds.device), corpus_embeds).detach()
                    cos_scores[torch.isnan(cos_scores)] = -1

                    # Get top-k values
                    cos_scores_top_k_values, cos_scores_top_k_idx = torch.topk(cos_scores,
                    ...
                    metric.add_batch(cos_scores_top_k_values=cos_scores_top_k_values,
